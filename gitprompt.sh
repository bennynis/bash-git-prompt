...
function updatePrompt() {
  ...
  git_repo=$(git remote -v|head -1|sed "s#.*/\(.*\)\.git.*#\1#")
  ...
  if [[ -n "$git_status_fields" ]]; then
    local STATUS="${PROMPT_LEADING_SPACE}${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}${GIT_BRANCH}${Cyan}@${Blue}${git_repo}${ResetColor}"
